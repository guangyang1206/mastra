...
## Custom handler context (`ctx`)

Added in: `@mastra/core@1.23.0`

Custom channel handlers (`onDirectMessage`, `onMention`, `onSubscribedMessage`) receive a 4th `ctx` argument that gives you first-class access to the active agent run on the current thread.

The `ctx` object exposes:

- `threadId` / `resourceId` — the Mastra thread + resource ids resolved for this chat thread (memory & history scope)
- `abortSignal: AbortSignal` — fires when the active run on this thread is aborted from anywhere (this handler, another handler on the same thread, `agent.abortRunStream()`, or `channels.close()`)
- `abort(reason?): boolean` — abort the currently active run on this thread; returns `true` if a run was aborted
- `activeRunId(): string | undefined` — the active runId on this thread, or `undefined` if idle

### Stop handler example

This unblocks the user request in #17065 — a `'stop'` command handler is now this short:
...
### AbortSignal for async work

`ctx` is also a natural place to pass `abortSignal` into any async work a handler kicks off (`fetch`, model calls, etc.) so the work stops cleanly when the run is canceled:
...
The 4th `ctx` arg is optional from the caller's TypeScript perspective. Existing 3-arg handlers (`(thread, message, defaultHandler) => …`) compile and run unchanged.
